...
class SubscriptionMonitor {
  constructor() {
    ...
    console.log('Subscription Monitor initialized for account:', this.targetAccount);
  }
  ...
  async initialize(hiveMonitor) {
    ...
    // Clean up processing queue every 10 minutes
    setInterval(() => {
      this.cleanupProcessingQueue();
    }, 10 * 60 * 1000); // 10 minutes
    
    // Start payment notification service
    paymentNotificationService.startScheduledChecks();
  ...
  async processPayment(paymentId, paymentData) {
    try {
      ...
      console.log(`📝 Parsed memo:`, memoData);
      ...
      if (!tierMatch) {
        ...
        await paymentNotificationService.sendPaymentFailedNotification(
          from_account,
          { amount, currency },
          'Could not match payment to any subscription tier'
        );
        return;
      }
      ...
      if (memoData.promoCode) {
        ...
        if (promoResult.success) {
          finalAmount = this.calculateDiscount(expectedAmount, promoResult.promoCode);
          promoCode = promoResult.promoCode;
          console.log(`🎟️ Applied promo code: ${promoCode.code} (${finalAmount} ${currency})`);
        }
      }
      
      // Check if payment amount matches (with 2% tolerance for fees)
      const tolerance = finalAmount * 0.02;
      const amountMatches = Math.abs(amount - finalAmount) <= tolerance;
      
      if (!amountMatches) {
        const errorMessage = `Amount mismatch: expected ${finalAmount} ${currency}, got ${amount} ${currency}`;
        await this.updatePaymentStatus(paymentId, 'failed', errorMessage);
      ...
      if (existingSubscription && memoData.renewal) {
        ...
        const renewedSubscription = await this.getSubscriptionById(existingSubscription.id);
        if (renewedSubscription) {
          await paymentNotificationService.sendSubscriptionRenewedNotification(
            renewedSubscription,
            {
              amount: finalAmount,
              currency: currency,
        ...
        console.log(`✨ Payment ${transaction_id}: Created subscription ${subscriptionId} for ${from_account}`);
        
        // Send subscription created notification
        const newSubscription = await this.getSubscriptionById(subscriptionId);
        if (newSubscription) {
          await paymentNotificationService.sendSubscriptionRenewedNotification(
            newSubscription,
            {
      ...
      await this.updatePaymentStatus(paymentId, 'processed', null, null);
  ...
  async matchTierByAmount(amount, currency, isYearly = false) {
    ...
    const query = `
      SELECT * FROM subscription_tiers 
      WHERE ${priceColumn} > 0 
        AND ABS(${priceColumn} - $1) <= ${priceColumn} * 0.02
        AND is_active = true
  ...
  cleanupProcessingQueue() {
    ...
    for (const [txId, timestamp] of this.processingQueue.entries()) {
